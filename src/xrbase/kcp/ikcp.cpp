...
const IUINT32 IKCP_DEADLINK = 300;
...
ikcpcb* ikcp_create(IUINT32 conv, void* user)
{
	...
	kcp->buffer = (char*)ikcp_malloc(((size_t)kcp->mtu + IKCP_OVERHEAD) * 3);
	if (kcp->buffer == NULL) {
		ikcp_free(kcp);
		return NULL;
	}
...
{
	size_t newsize = (size_t)kcp->ackcount + 1;
	IUINT32* ptr;
...
int ikcp_setmtu(ikcpcb* kcp, int mtu)
{
	...
	if (mtu < 50 || mtu < (int)IKCP_OVERHEAD)
		return -1;
	buffer = (char*)ikcp_malloc(((size_t)mtu + IKCP_OVERHEAD) * 3);
	if (buffer == NULL)
		return -2;
	kcp->mtu = mtu;
	kcp->mss = kcp->mtu - IKCP_OVERHEAD;
